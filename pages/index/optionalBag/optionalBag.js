...
Page({
  data: {
    ...
    imgUrls: [
      "/images/tob_zx_logo1.png",
      "/images/tob_zx_logo2.png",
      "/images/tob_zx_logo3.png"
    ], //头部logo img
  ...
  //商品选择 材料材质处理函数  暂时不谢
  // switchMaterial: function (e) {
  //   var that = this;
  //   let material = e.target.dataset.material;
  //   // console.log(color);
  //   that.setData({
  //     material: material, //嘴中选择的颜色
  //   })
  // },
  //商品选择 动态属性处理函数
  ...
  addBtn: function (e) {
    ...
    if (showThis == 'right' || showThis == 'left' || showThis == undefined) {
      that.setData({
        mask_show: false,
        rightSlide: 'left'
      })
  ...
  reload: function () {
    if (getCurrentPages().length != 0) {
      getCurrentPages()[getCurrentPages().length - 1].onLoad(shareMessageArg)
    }
